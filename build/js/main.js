...
function getProfilePhoto(image, gender) {
  if(image){
    ...
    if(gender == "male") {
      return <img src='https://joshwilkerson.com/PCO/users/default-user-m.jpg' />;
    } else {
      return <img src='https://joshwilkerson.com/PCO/users/default-user-f.jpg' />;
    }
  }
}
...
function formatPhoneNumber(phone) {
   ...
   let phone1 = phoneStr.substr(0,3);
   let phone2 = phoneStr.substr(3,3);
   let phone3 = phoneStr.substr(6,4);
  ...
  const showUser = userData.map((user) =>
    <div class='user'>
      <a href="#">
        ...
        <span class='phone'>{formatPhoneNumber(user.phone.primary)}</span>
        <span class='email'>{user.email.primary}</span>
        <span class='address'>{user.address.primaryStreet} {user.address.primaryCity}, {user.address.primaryState} {user.address.primaryZip}</span>
      </a>
    </div>
  );
  return (
...
ReactDOM.render(
  ...
  document.getElementById('results--items')
);
